...
function initialTblTop() {
    ...
    var where = "file_status='未确认' and file_no in (select DISTINCT ralate_no from apo_act where apo_finished='0' and (next_user_name='"+userName+"' or ( act_step='0' and act_user_no='"+userNo+"')))";
    $('#TblTop').datagrid({
        ...
        pageNumber: 1,
        pageList: [10, 20, 30],
        queryParams: { viewNeedOperate: 'dmsFile', orderBy: JSON.stringify({ eqm_no: 'desc', file_type_no: 'desc' }), where: strBase64(where) },
        columns: [[
...
function topTblRefresh() {
    ...
    var where = "file_status='未确认' and file_no in (select DISTINCT ralate_no from apo_act where apo_finished='0' and (next_user_name='" + userName + "' or ( act_step='0' and act_user_no='" + userNo + "')))";
    var queryParams = { viewNeedOperate: 'dmsFile', orderBy: JSON.stringify({ file_type_no: 'desc', eqm_no: 'desc' }), where: strBase64(where) }
...
//                var file = document.getElementById("filebox_file_id_3").files[0];
//                var fd = new FormData();
