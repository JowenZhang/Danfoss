...
function initialTblTop() {
    $('#TblTop').datagrid({
        ...
        pageNumber: 1,
        pageList: [10, 20, 30],
        queryParams: { viewNeedOperate: 'eqmJamCause', orderBy: JSON.stringify({ jam_cause_no: 'desc' }), where: strBase64('') },
        columns: [[
            { field: 'jam_cause_no', title: '停机编码', align: 'left', width: 140 },
...
function topEditClick() {
    ...
    $('#TxtJamCauseNo').textbox('setText', row.jam_cause_no);
    $('#TxtJamCauseNo').textbox('disable');
    $('#TxtJamCauseName').textbox('setText', row.jam_cause_name);
    ...
    $('#TopEditFrm').dialog({
        ...
        buttons: [{
            text: '确认',
            handler: function () {
                row.jam_cause_no = $('#TxtJamCauseNo').textbox('getText');
                row.jam_cause_name = $('#TxtJamCauseName').textbox('getText');
                row.status_no = $('#CmbStatusTop').combobox('getValue');
                row.status_name = $('#CmbStatusTop').combobox('getText');
                $.post('/BackgroundProgram/EditObject.ashx', { viewNeedOperate: 'eqmJamCause', objNeedOperate: JSON.stringify(row) }, function (data) {
                    if (data === null) {
...
function topAddClick() {
    $('#TopEditFrm').dialog({
        ...
        buttons: [{
            text: '确认',
            handler: function () {
                var jam_cause_no = $('#TxtJamCauseNo').textbox('getText');
                var jam_cause_name = $('#TxtJamCauseName').textbox('getText');
                var status_no = $('#CmbStatusTop').combobox('getValue');
                var status_name = $('#CmbStatusTop').combobox('getText');
                $.post('/BackgroundProgram/AddObject.ashx', { viewNeedOperate: 'eqmJamCause', objNeedOperate: JSON.stringify({ jam_cause_no: jam_cause_no, jam_cause_name: jam_cause_name, status_no: status_no, status_name: status_name }) }, function (data) {
                    if (data === null) {
                        $.messager.alert('提示信息', '无法获取结果！', 'error');
                        return;
                    }
...
function topDeleteClick() {
    ...
    $.messager.confirm('确认', '确认要删除这些数据？', function (r) {
        ...
        $.post('/BackgroundProgram/DeleteObject.ashx', { viewNeedOperate: 'eqmJamCause', objNeedOperate: JSON.stringify(rows) }, function (data) {
            if (data === null) {
                $.messager.alert('提示信息', '无法获取结果！', 'error');
                return;
            }
