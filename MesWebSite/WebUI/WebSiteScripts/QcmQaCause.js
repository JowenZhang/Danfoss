...
function initialTblTop() {
    $('#TblTop').datagrid({
        ...
        pageNumber: 1,
        pageList: [10, 20, 30],
        queryParams: { viewNeedOperate: 'qcmQaCause', orderBy: JSON.stringify({ qa_cause_no: 'desc' }), where: strBase64('') },
        columns: [[
            { field: 'qa_cause_no', title: '异常编码', align: 'left', width: 140 },
...
function topEditClick() {
    ...
    $('#TxtQaCauseNo').textbox('setText', row.qa_cause_no);
    $('#TxtQaCauseNo').textbox('disable');
    $('#TxtQaCauseName').textbox('setText', row.qa_cause_name);
    ...
    $('#TopEditFrm').dialog({
        ...
        buttons: [{
            text: '确认',
            handler: function () {
                row.qa_cause_no = $('#TxtQaCauseNo').textbox('getText');
                row.qa_cause_name = $('#TxtQaCauseName').textbox('getText');
                row.status_no = $('#CmbStatusTop').combobox('getValue');
                row.status_name = $('#CmbStatusTop').combobox('getText');
                $.post('/BackgroundProgram/EditObject.ashx', { viewNeedOperate: 'qcmQaCause', objNeedOperate: JSON.stringify(row) }, function (data) {
                    if (data === null) {
...
function topAddClick() {
    $('#TopEditFrm').dialog({
        ...
        buttons: [{
            text: '确认',
            handler: function () {
                var qa_cause_no = $('#TxtQaCauseNo').textbox('getText');
                var qa_cause_name = $('#TxtQaCauseName').textbox('getText');
                var status_no = $('#CmbStatusTop').combobox('getValue');
                var status_name = $('#CmbStatusTop').combobox('getText');
                $.post('/BackgroundProgram/AddObject.ashx', { viewNeedOperate: 'qcmQaCause', objNeedOperate: JSON.stringify({ qa_cause_no: qa_cause_no, qa_cause_name: qa_cause_name, status_no: status_no, status_name: status_name }) }, function (data) {
                    if (data === null) {
                        $.messager.alert('提示信息', '无法获取结果！', 'error');
                        return;
                    }
...
function topDeleteClick() {
    ...
    $.messager.confirm('确认', '确认要删除这些数据？', function (r) {
        ...
        $.post('/BackgroundProgram/DeleteObject.ashx', { viewNeedOperate: 'qcmQaCause', objNeedOperate: JSON.stringify(rows) }, function (data) {
            if (data === null) {
                $.messager.alert('提示信息', '无法获取结果！', 'error');
                return;
            }
